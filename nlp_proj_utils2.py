...
def _download_data_if_needed(data_meta: DataMeta) -> str:
    """
    Download and extract dataset if needed return the path to the dataset
    """
...
def load_glove_vecs() -> Tuple[Dict[str, int], Dict[str, np.array]]:
    ...
    2. Word to vector, mapping from word to its vector
    """
    ...
    with open(path, encoding="utf-8") as f:
        ...
        for line in f:
            ...
            cur_word = line_components[0]
            words.add(cur_word)
            word_to_vec[cur_word] = np.array(line_components[1:], dtype=np.float64)
        i = 1
        word_to_index = {}
...
def plot_decision_boundary(model, X: np.array, y):
